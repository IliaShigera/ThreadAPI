...
internal static class ServiceCollectionExtensions
{
    ...
    internal static AuthenticationBuilder AddJwtAuth(this IServiceCollection services, IConfiguration configuration)
    {
        var key = Encoding.ASCII.GetBytes(configuration.GetValue<string>("AuthConfig:JwtSecretKey"));
        return services.AddAuthentication(config => { config.DefaultScheme = JwtBearerDefaults.AuthenticationScheme; })
            .AddJwtBearer(config =>
            {
